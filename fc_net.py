    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == "batchnorm":
                ...
                caches.append(('bn', cache_bn))
            
            out, cache_relu = relu_forward(out)
            caches.append(('relu', cache_relu))
            
            if self.use_dropout:
                out, cache_dropout = dropout_forward(out, self.dropout_param)
                caches.append(('dropout', cache_dropout))
        
        out, cache_final = affine_forward(out, self.params[f'W{self.num_layers}'], 
                                           self.params[f'b{self.num_layers}'])
        scores = out
        caches.append(('affine_final', cache_final))
        ...
        cache_type, cache_final = caches.pop()
        dout, dw_final, db_final = affine_backward(dout, cache_final)
