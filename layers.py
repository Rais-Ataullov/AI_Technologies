...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x_norm, x_centered, sample_var, sample_mean, gamma, eps, x)
...
def batchnorm_backward(dout, cache):
    ...
    x_norm, x_centered, sample_var, sample_mean, gamma, eps, x = cache
    N, D = dout.shape
    ...
    dx_centered = dx_norm * inv_std
    dvar = np.sum(dx_norm * x_centered * (-0.5) * inv_std**3, axis=0)
    dmean = np.sum(-dx_centered, axis=0) + dvar * np.mean(-2 * x_centered, axis=0)
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dx[n, c, h_start:h_end, w_start:w_end] += dout[n, c, i, j] * mask / np.sum(mask)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (x, x_norm, mean, var, gamma, G, eps, N, C, H, W)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, x_norm, mean, var, gamma, G, eps, N, C, H, W = cache
    ...
    dx_reshaped = dx_norm_reshaped * inv_std
    
    dvar = np.sum(dx_norm_reshaped * (x_reshaped - mean) * (-0.5) * inv_std**3, axis=(2, 3, 4), keepdims=True)
    dmean = np.sum(-dx_reshaped, axis=(2, 3, 4), keepdims=True) + dvar * np.mean(-2 * (x_reshaped - mean), axis=(2, 3, 4), keepdims=True)
    
    dx = dx_reshaped + dvar * (2 * (x_reshaped - mean) / (C // G * H * W)) + dmean / (C // G * H * W)
